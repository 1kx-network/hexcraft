...
contract ClericsTent is BuildingKind {
    function use(Game ds, bytes24 buildingInstance, bytes24 actor, bytes memory /*payload*/ ) public {
        ...
        require(!(UnitIsCarryingItem(state, actor, FRIENDLY_PUPPY_COMPANION_ITEM_ID)), "You already own a Friendly Puppy Companion");
        require(!(UnitIsCarryingItem(state, actor, GLOCK_WITH_MAGIC_ITEM_ID)), "You are already carrying a Glock 19 with magic");
        require(!(UnitIsCarryingItem(state, actor, GREATSWORD_OF_DAWN_ITEM_ID)), "You are already carrying the greatsword of dawn");
